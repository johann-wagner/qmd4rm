...
Arguments should include - `data` - the input data frame, in tidy format - `row` - use `filter()` for row selection - no default - `column` - use `select()` for column selection - no default - `unit` - to define the unit of the value being returned (n, %, other?) - no default? - `digits` - define the number of significant digits to round to - default 1 - `smallcell` - define the value at which cell suppression occurs - default FALSE - `allowable_range` - two-value argument defining the upper and lower bounds the value must fall within - default FALSE
...
The number of people living regionally
`r get_value(df, count, (rurality == 'Regional' & quintile == 5))`
